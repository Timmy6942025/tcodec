...
/* TRIAL62 RETEST (trial 35 lost pre-RDOQ; world changed since):
 * count only coded positions [0..last]. Both coders stop at last_nz. */
static int count_siglevel_bits(const tc_coeff_t *c, int last)
{
    int bits = 0;
...
static int64_t qt_leaf(qt_enc_t *e, int depth, int cx, int cy, int write)
{
    ...
    if (e->frame_type == TC_FRAME_BIDIR) {
        ...
        /* Search range by preset (trial 48/49): 16 fast, 24 medium/slow.
         * Wider ranges lose (SAD-best ≠ RD-best; mismatch grows with
         * range — slow sr64 measured +3.2%/flat vs medium on park). */
