...
| Regression suite | 52 codec tests plus container/integration tests |
...
make test                    # bounded 51-test regression + containers
make test-full               # all 52 codec tests, including 300-frame long run (slow)
...
`test/test_tcodec.c` currently registers 52 tests covering color conversion,
...
`make test` is the bounded default: it runs 51 tests and explicitly reports the
single skipped 300-frame in-process long run. `make test-full` runs all 52.
