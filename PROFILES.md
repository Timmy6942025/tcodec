...
**Bitstream Versions**: 0 (no profile/level), 1 (profiles + levels + tool flags), 2 (explicit quadtree payload + per-row entry points), 2.1 (scoped NOT implemented: FRESH_SKIP kept firing 0%, MERGELIST/MPM/1-bit batched, see BITSTREAM §7.7 + docs/V2_1_BATCH.md)
**Last Updated**: v2 quadtree payload, luma SAO Band Offset, and per-row entry points (D9 parallel parse) are implemented; v2.1 remainder scoped (FRESH_SKIP kept, others batched, see BITSTREAM §7.7); EO/restoration remain reserved.
