...
| D9 | ARM decode ≥60fps@720p / ≥30fps@1080p | 🔶 12/15 clips @60+, 1080p at 25 | Quiet-box best-of-N internal, entry-points streams: 720p t4 ≥60fps on 12/15 clips (csgo, ed, tree, screen, sintel, sita, vidyo, old_town, stockholm, tos, park30 60.5, minecraft30 63.1); short: bbb30 55.6, parkrun30 56.9, ducks30 52.6 (grain/water volume program). 1080p t4 ~25fps (need 30). KEY: x264 itself decodes park q32 at 30.1fps best here — tcodec (24–31 contended) is at PARITY with the reference. Landed (all bit-exact, 54/54 + soak + parity + fuzz): O3/mcpu/LTO, persistent v2 cmds, per-frame + flat quant, tail memset, int-MV path, deblock hoist, range refill/divides, parse→recon pipeline, cost scheduler, v2 ENTRY_POINTS (§7.6.1). RDOQ-L2 trial reverted (L2s exist and are RD-worth-keeping; gate fired never). Program in docs/D9_PLAN.md |
