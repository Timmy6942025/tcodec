...
| D3 | Sanitizer clean + fuzz coverage documented | ✅ Done* | UBSan fast suite 53/53 zero findings (halt_on_error); valgrind memcheck clean on v2+entropy enc+dec; fuzz tests pass in-suite. *ASan cannot init on this box (container mmap restriction, pre-existing env blocker — precise cause documented in HILLCLIMB) |
...
| D8 | Real-content BD-rate vs x264/x265/SVT-AV1 | 🔄 In progress | 8-clip corpus fetched (manifest 8); pilot old_town qp32 all-4-codecs green; full 8×4×3QP matrix running; park BD-rate +99.6% vs x264vf recorded |
