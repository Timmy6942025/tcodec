...
| D9 | ARM decode ≥60fps@720p / ≥30fps@1080p | 🔶 In progress (sprint) | Best contended-box internal: park q32 med t1/t4 ~18/24fps (was 16.9/18.0), q37 matrix 30–48fps over 15 clips (need 60); 1080p sintel/tos fast q32 t4 ~14–15fps (need 30). KEY: x264 itself decodes park q32 at 30.1fps best here — tcodec (24–31fps) is at PARITY with the reference; the 60fps gap is hardware-dimensional on this saturated box (avg ~10), not a code defect. Landed (all bit-exact, 53/53): O3/mcpu/LTO flags, persistent v2 cmds (no 15MB/frame calloc), per-frame quant tables + flat dequant, coeff tail-memset, motion int fast path, deblock threshold hoist, range refill/divide fixes, parse→recon pipeline, cost-aware wavefront, v2 per-row ENTRY_POINTS (parallel parse, §7.6.1). Scaling proof needs quiet hardware. Program in docs/D9_PLAN.md |
