...
| D9 | ARM decode ≥60fps@720p / ≥30fps@1080p | 🔶 12/15 + q37 rung; STABLAM reverted | Quiet-box best-of-N internal: 720p t4 ≥60fps on 12 clips (clean q32 EP); park/parkrun/ducks need volume work (STABLAM gave 60.1/65.5/60.5 but bloated structured content 2x+ at identical PSNR — SAD cannot separate water from UI motion; reverted, revival needs motion-AND-texture gating). Entry-point SIZE gate landed instead (micro-rows serial: screen/sita restored, park stays EP). 1080p t4 34–36fps on q37 rung (need 30 ✅), 25–27 on q32. x264 parity at 30fps park. Landed (all bit-exact, 54/54 + soak + parity + fuzz): O3/mcpu/LTO, persistent v2 cmds, flat quant, tail memset, int-MV path, deblock hoist, range refill/divides, parse→recon pipeline, cost scheduler, v2 ENTRY_POINTS (§7.6.1, size-gated). RDOQ-L2/3L, LAM sweeps, MVDBITS, KEYQP, MERGETU, chaos-tier reverted with proof. Bisect cleared hills 47/48, convicted hill-49 (reverted). Program in docs/D9_PLAN.md |
