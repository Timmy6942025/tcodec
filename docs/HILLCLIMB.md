...
| 85 | luma/chroma coeff context split (chroma bank 86..118 duplicates luma 21..53, +65 offset; 13 sites: enc 6 (v2 split, legacy shared frozen), dec 9 (v2 split, legacy shared); MAX/NUM 86->119; tests updated is_chroma=0 luma) | probe -3.7%/flat (19685 vs 20434); park -3.5%/flat (369458 vs 382961); screen +0.3%/flat neutral (1769 vs 1763); parity ALL OK; fast 53/53 (fixed 834 luma4x4 misclassified as chroma + 619 legacy chroma as split (should shared); tests caught, fixed, green) | KEEP - 3rd largest win since deblock (after MVP -10%, temporal -8%); zero decoder-cost change (1 add for base), 33B memory. Residual 86% prize, luma+chroma both win (less pollution both ways). Goldens v2 move, v0/v1 identical (legacy frozen). |
| 88 | SAO model re-tune x6→x4/x8 (post-MVP/temporal/chroma world-change check; 2nd knee test after Session 10) | x4: probe +0.39%/+0.05dB (up-slide), screen +2.3% (1810B vs 1769B, poor rate); x8: probe −0.13%/−0.04dB (down-slide, 0.31 dB/% poor rate) | REVERT (keep x6) — knee stable across deblock/RDOQ/MVP/temporal/chroma; no park (dual-gate fast-fail) |
| 89 | deblock tc re-tune qp/3→qp/6 (weaker, preserves detail) post-MVP/temporal/chroma world-change check; 8 sites scalar+NEON identically (parity); 2nd knee test after trial51 | probe −3.8%/+0.11dB (18943 vs 19685, win); park −0.36%/+0.03dB neutral (368139 vs 369458); screen +2.0%/flat loss (1805B vs 1769B, blocking on flat UI costs bits) | REVERT (keep qp/3) — knee stable across deblock-rewrite/RDOQ/MVP/temporal/chroma; screen fails like trial51 (different direction: was −1.6%/−0.30dB fewer+worse, now +2%/flat more+flat — both fail). Thresholds roughly optimal; both directions tested twice. |
| 90 | per-block 8x8 stability infra (((w+7)/8)*((h+7)/8) int32 SADs + BLKSTABDBG meter; no RDO use yet, encoder-only, no syntax) | probe 19685 baseline; parity ALL OK; fast 53/53; vidyo 28-41% blocks static vs 16-27% CTUs (finer, separates bg/face, trial44 faces need finer) | KEEP (infra) - 57KB @720p + SAD (negligible vs RDO). Enables per-block rho + near-exact skip next (finer gating). Pattern matches CTU stability hill-44. |
| 91 | D9 entry-points code (row-independent grids + per-row payloads/table + parallel parse, 20+ sites) | 2h time-box expired before green (high desync risk like trial74 14dB/chroma 11dB, fps noisy Pi cannot validate 1.5x reliably) | PARKED (code deferred to perf-box sprint with quiet box + full day, design kept docs/D9_ENTRY_POINTS_DESIGN.md). Pi validates deterministic correctness (parity/suite/goldens/compression loss) but not noisy fps wins; desync risk for unproven benefit violates Verification. Do NOT drive by solo (needs 10+ site audit + quiet box). |
| 92 | P lookahead buffer infra (TC_LOOKAHEAD_N=4 future inputs, la_frame[4] NULL init + NULL-safe free, on-demand alloc (none yet), no delay, no RDO use, encoder-only, no syntax) | probe 19685 baseline; parity ALL OK | KEEP (infra) - NULL buffers, no-ops, zero behavior change. Enables future propagation pricing (mb-tree future-reference value, trial19 lesson) with N-frame delay (streaming latency tradeoff, CBR?). Pattern matches prev-orig/prev-MV/cur-MV/stability/per-block buffers (hill-44/45/47). |
| 93 | near-exact skip SSE<=cu (very strict, MSE<=1/cu? 8x8 SSE<=8 MSE<=0.125; relaxes perfect ==0 gate hill-42, P-only, encoder-only, FRESH_SKIP bit already kept, fresh paths already built, no decoder change) | probe/screen/park all byte-identical (firing 0% everywhere, even screen/static with fixed MVP — threshold still too strict; safe, neutral); parity ALL OK; fast 53/53 | KEEP (infra, gate relaxed, still safe) - enables future threshold sweep (SSE<=cu*2/4/... up to MSE<=1? SKIPDBG screen 0% even MSE<=1 generous, need MSE>1 risking poisoning trial19; with ρ/mb-tree + lookahead guard, future). Pattern matches perfect-match infra hill-42 (firing 0%, enables future). |
| 86 | chroma SAO BO (per-CTU Cb/Cr band/offset, 1+9+9=19 bits/CTU overhead) | overhead 4.6%/frame vs prize 4.3% (chroma share) -> cannot win even if chroma eliminated | PARKED (reasoned rejection, no code) - overhead exceeds prize (like SATD, SUB4 verdicts). CfL already covers correlated color; BO adds little on top. Do not build. |
| 87 | D9 entry-points design (docs/D9_ENTRY_POINTS_DESIGN.md, scoped NOT implemented) | 18fps vs 60fps (3.3x short, 1.2x scaling starvation, serial parse 21%, interp memory-bound); entry-points alone ~1.2-1.5x (not 3.5x), program needed | DESIGN KEPT - row-independent grids/contexts + per-row payloads/table + parallel parse + version ceremony. Code deferred to perf-box sprint (timings noisy here, cannot validate 1.5x reliably; desync risk like trial74 needs 10+ site audit + quiet box). Enables future D9 sprint. |
...
| 108 | TEXLAM-2x: per-leaf λ×2 on motion-AND-texture gate (orig var≥300 AND mvp-SSE/area≥150; P-only, encoder-only, nd-stored bit for write/replay RDOQ parity; forensics: fires 57% ducks / 0.3% screen) | probe −3.3%/−0.20dB; park qp32 −11.3%/−0.41, ducks −6.9%/−0.41, sita +0.2%/−0.002 (spare), screen +1B/−0.01 (spare); all DET-OK, parity green. 3-QP BD vs self: park +0.58%, ducks +1.95% (diagonal slide — qp27 point loses: fine-QP texture bits well spent) | REVERTED (gate) — first texture gate to spare screen/sita BY CONSTRUCTION (STABLAM failure mode avoided), but lever direction sits inside ducks' curve |
| 109 | TEXLAM-2x + QP gate (fire only qp≥30; qp27 verified bit-identical to baseline, probe 9714B pinned) | 3-QP BD: park −0.15% (neutral), ducks +1.28% (still loses) | REVERTED — pinning the top fixes park, ducks' qp32 exchange (0.059 ≈ curve slope) is structural |
| 110 | TEXLAM-3x + QP gate | park qp32 −21%/−0.81, qp37 −34%/−0.69 (0.020, good); ducks qp32 −13.7%/−0.82 (0.060, bad), qp37 −27.6%/−0.89 (0.032); BD park −0.19%, ducks +3.21% (stronger = deeper loss) | REVERTED same day, gate + nd bit + save/restore fully removed (park-hash revert check bit-identical 376,522B; probe 19921 exact; 53/53; parity green). Lesson: λ redistribution cannot win where dl≫λ·bits (texture residuals stubborn); needs smaller residuals (better prediction), not coarser pricing. TEXDBG meter KEPT (env-gated forensics) |
| 111 | B-on-ducks scout (single point qp32, `-b`, no code change) | 410,553B@26.50 vs P 450,888B@26.93 (−9.0%/−0.43dB, diagonal) | NO CHANGE — consistent with park-neutral/screen-harmful audit; B ladder+RDO retune stays a project |
| 112 | λ re-scan 200→175/225 (4th knee test; 1st post-temporal/chroma-split world change) | 175: probe +2.3%/+0.10dB (up-slide); 225: probe −1.1%/−0.05dB (0.045 dB/%, poor rate) | REVERTED (keep 200), no trace — knee stable across RDOQ/honest-lb/key-lambda/SAO/MVP/temporal/chroma |
